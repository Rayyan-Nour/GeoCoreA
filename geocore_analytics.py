...
DEFAULT_RESULTS = str(Path(DEFAULT_BASE) / "results_v4")
DEFAULT_OVERLAY = "oreinsight_v4_probability.tif"
...
class PipelineWorker(QtCore.QThread):
    ...
    def _stage_progress(self, line_lower: str):
        ...
        if "loading deposits" in line_lower or "loading copper" in line_lower or "loading known" in line_lower:
            return 25, "Loading deposit locations…"
        ...
        try:
            ...
            for raw_line in proc.stdout:
                ...
                if mapped is not None:
                    ...
                    # Do not inflate progress from arbitrary log chatter.
                    # Progress should come from explicit pipeline markers.
                    pass
...
class GeoCoreAnalyticsMainWindow(QtWidgets.QMainWindow):
    ...
    def _update_inline_progress(self, value, message):
        ...
        elif value >= 10 and self._analysis_start_time:
            elapsed = time.time() - self._analysis_start_time
            if elapsed < 8:
                self.inline_eta_label.setText("Estimating...")
                return
    ...
    def _build_dynamic_report(self, validation_text, features_list):
        ...
        # Detect commodity
        m_commodity = re.search(r'Commodity: (\w+)', validation_text)
        if m_commodity:
            report_commodity = m_commodity.group(1).lower()
        elif any('radiometric' in f.lower() or 'ree' in f.lower() for f in features_list):
            report_commodity = "ree"
        else:
            report_commodity = "copper"
        ...
        if report_commodity == "ree":
            lines.append("Mineral Target:   Rare Earth Elements (REE) - Carbonatite Systems")
        else:
        ...
        if is_transfer:
            ...
            dep_label = "REE" if report_commodity == "ree" else "Cu"
            lines.append("Trained on {} known {} deposits with {} feature layers.".format(n_deposits, dep_label, n_feat))
        ...
        if report_commodity == "ree":
            lines.append("   Grade Model:  Carbonatite REE reference (Verplanck et al., 2014)")
            lines.append("                 Range: 1-10% TREO | Mountain Pass: ~7% TREO")
        else:
        ...
        if report_commodity == "ree":
            lines.append("   - Lawley et al. (2024) Natural Resources Research - Canadian carbonatite REE")
            lines.append("   - Verplanck et al. (2014) USGS - Carbonatite REE deposit model")
            lines.append("   - Bishop & Robbins (2024) J. Geochem Exploration - Th/P as REE predictors")
            lines.append("   - Goldman & Ellefsen (2020) USGS - Bayesian NURE radiometric grids")
        else:
        ...
        if is_transfer:
            ...
            if report_commodity == "ree":
                lines.append("   5. Target depth: 0-500m for carbonatite REE (typically near surface)")
            else:
    ...
    def _save_report_txt(self, report_text):
        ...
        if filename:
            ...
            project_data = {
                ...
                'deposit_type_map': os.path.join(cfg['results'], 'oreinsight_v4_deposit_type.tif')  # Legacy
            }
...
• Version history
    ...
    def show_model_development(self):
        ...
        try:
            ...
            model_path = os.path.join(cfg.get("results", "results_v4"), "oreinsight_v4_copper_model.pkl")
            validation_path = os.path.join(cfg.get("results", "results_v4"), "oreinsight_v4_validation.txt")
